...
readonly class OutputDetailsDto implements Arrayable
{
    ...
    {
        $vendedor_id = (int)($chat->producto?->vendedor_id
            ?? $chat->producto?->vendedor?->id
            ?? 0);
        ...
        return new self(
            ...
            producto: $chat->relationLoaded('producto') && $chat->producto
                ? [
                    ...
                    'imagen' => $chat->producto->relationLoaded('fotos') && $chat->producto->fotos->isNotEmpty() && $chat->producto->fotos->first()?->imagen
                        ? asset($chat->producto->fotos->first()->url)
                        : null,
                    'vendedor' => $chat->producto->relationLoaded('vendedor') && $chat->producto->vendedor && !$bloqueo_mutuo
                        ? [
                            'id'       => $chat->producto->vendedor->id,
                            'nickname' => $chat->producto->vendedor->nickname,
                            'imagen'   => asset($chat->producto->vendedor->imagen ?? 'storage/usuarios/default.png'),
                        ]
                        : [],
                ]
                : [],
