...
<body style="display:flex;align-items:center;justify-content:center;min-height:100vh;flex-direction:column;gap:1rem;">
    ...
    <script>
        (function() {
            ...
            var url = (window.API_CONFIG && window.API_CONFIG.LARAVEL_URL) ? window.API_CONFIG.LARAVEL_URL + 'productos/' + productoId + '/chats' : '<?= (defined('LARAVEL_API_URL') ? LARAVEL_API_URL : 'http://localhost:8000/api/') ?>productos/' + productoId + '/chats';
            fetch(url, {
                method: 'POST',
            ...
            .then(function(data) {
                ...
                if (chatId) {
                    ...
                    document.body.innerHTML = '<p>No se pudo iniciar el chat. <a href="<?= $base ?>productos/producto.php?id=<?= $producto_id ?>">Volver al producto</a></p>';
                }
            })
            .catch(function() {
