...
try {
    ...
    $countQuery = "SELECT COUNT(DISTINCT p.id) as total
        ...
        WHERE p.estado_id = 1 AND u.estado_id = 1 AND p.vendedor_id != ?
        AND p.vendedor_id NOT IN (SELECT bloqueado_id FROM bloqueados WHERE bloqueador_id = ?)
        AND p.id NOT IN (SELECT producto_id FROM chats WHERE estado_id IN (5, 8))";
    // 5 = vendido (compra confirmada), 8 = devuelto (devolución) → no mostrar en home
